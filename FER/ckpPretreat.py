...
dataset_path = "ClassifiedDataCK+/2"
ckp_path = "ckp"
images_path = "ckp"
        ...
        for filename in os.listdir(label_path):
            ...
            for i in range(5):
                ...
                if len(face) > 0:
                    ...
                    face_img2 = cv2.resize(face_img, (48, 48))
                    ...
                    cv2.imwrite(save_image_path, face_img2)
                    # 获取嘴部区域
                    mouth_x = face[0]['keypoints']['mouth_left'][0]
                    mouth_y = min(face[0]['keypoints']['mouth_left'][1], face[0]['keypoints']['mouth_right'][1])
                    mouth_img = augmented_image[mouth_y-30:mouth_y + 40, mouth_x:face[0]['keypoints']['mouth_right'][0]]
                    ...
                    cv2.imwrite(save_image_path, mouth_img)
                    # 获取眼部区域
                    eye_x = face[0]['keypoints']['left_eye'][0]
                    eye_y = min(face[0]['keypoints']['left_eye'][1], face[0]['keypoints']['right_eye'][1])
                    eyes_img = augmented_image[eye_y-20:eye_y + 30, eye_x-50:face[0]['keypoints']['right_eye'][0]+50]
                    ...
                    cv2.imwrite(save_image_path, eyes_img)
...
def load_data():
    ...
    mouth_images = []
    mouth_labels = []
    for sort in os.listdir(ckp_path):
        sort_path = os.path.join(ckp_path, sort)
        for label in os.listdir(sort_path):
            label_path  = os.path.join(sort_path, label)
            for filename in os.listdir(label_path):
                image_path = os.path.join(label_path, filename)
                image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
                ...
                if sort == "eyes":
                    ...
                    mouth_images.append(image)
                    mouth_labels.append(emotion_labels[label])
    return np.array(face_images), np.array(face_labels), np.array(eye_images), np.array(eye_labels), np.array(
        mouth_images), np.array(mouth_labels)
